...
class _EllipticCurvePrivateKey(ec.EllipticCurvePrivateKey):
    ...
    def sign(
        ...
        if isinstance(signature_algorithm.algorithm, Prehashed):
            if len(data) != signature_algorithm._algorithm.digest_size:
                raise ValueError(
                    "The provided data must be the same length as the hash "
                    "algorithm's digest size."
                )
            evp_md = self._backend._ffi.NULL
        else:
            evp_md = self._backend._evp_md_from_algorithm(
        ...
        res = self._backend._lib.EVP_DigestSignUpdate(
            evp_md_ctx, data, len(data)
        )
        self._backend.openssl_assert(res == 1)
        siglen_ptr = self._backend._ffi.new("size_t *", 0)
        res = self._backend._lib.EVP_DigestSignFinal(
            evp_md_ctx, self._backend._ffi.NULL, siglen_ptr
        )
        self._backend.openssl_assert(res == 1)
        self._backend.openssl_assert(siglen_ptr[0] > 0)
        sigbuf = self._backend._ffi.new("unsigned char[]", siglen_ptr[0])
        res = self._backend._lib.EVP_DigestSignFinal(
            evp_md_ctx, sigbuf, siglen_ptr
        )
        self._backend.openssl_assert(res == 1)
